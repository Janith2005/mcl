...
Includes all GVB functions: _extract_stems, _count_pain_point_matches,
stem-based keyword matching, and dual-layer competitor bonuses.
"""
import re
...
def _extract_stems(text: str) -> List[str]:
    ...
    Ported from GVB scoring/engine.py.
    """
...
def _count_keyword_matches(text: str, keywords: List[str]) -> int:
    ...
    Ported from GVB: uses stem-based matching for better recall.
    """
...
def score_content_gap(text: str, brain_ctx: dict) -> int:
    ...
    Ported from GVB: uses pillar keyword matching for gap detection.
    """
    score = 6
    text_lower = _tokenize(text)
...
def score_proof_potential(text: str) -> int:
    ...
    Ported from GVB: uses action/opinion keyword distinction.
    """
    text_lower = _tokenize(text)
...
def apply_competitor_bonuses(scores: dict, views: int) -> dict:
    ...
    Ported from GVB: bonuses target content_gap and proof_potential (not icp_relevance).
    """
